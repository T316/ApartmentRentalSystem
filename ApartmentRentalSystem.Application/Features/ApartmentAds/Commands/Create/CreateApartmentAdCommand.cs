    ...
    public class CreateApartmentAdCommand : IRequest<CreateApartmentAdOutputModel>
    {
        ...
        public class CreateApartmentAdCommandHandler : IRequestHandler<CreateApartmentAdCommand, CreateApartmentAdOutputModel>
        {
                ...
                CancellationToken cancellationToken)
            {
                ...
                var broker = await this.brokerRepository.FindByUser(userId, cancellationToken);

                var category = await this.apartmentAdRepository.GetCategory(request.Category, cancellationToken);

                var neighborhood = await this.apartmentAdRepository.GetNeighborhood(request.Neighborhood, cancellationToken);

                var factory = neighborhood == null
                    ? this.apartmentAdFactory.WithNeighborhood(request.Neighborhood)
                    : this.apartmentAdFactory.WithNeighborhood(neighborhood);
